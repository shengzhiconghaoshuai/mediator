	...
	function addDiyDom(treeId, treeNode) {
		...
		if (treeNode.isKeyProp) {
			var editStr = "<span style='color:#cb6fd7;'>(关键属性)</span>";
			aObj.after(editStr);
		}
		if (treeNode.isSaleProp) {
			var editStr = "<span style='color:#3a87ad;'>(销售属性)</span>";
			aObj.after(editStr);
		}
		if (treeNode.must) {
	...
	function mappingAttr(event, treeId, treeNode) {
		...
		var tbKeyProp = treeNode.isKeyProp;
		var tbSalesProp = treeNode.isSaleProp;
		var tbColorProp = treeNode.isColorProp;
		var tbEnumProp = treeNode.isEnumProp;
		var tbItemProp = treeNode.isItemProp;
		var tbReqProp = treeNode.must;
		var tbMulti = treeNode.multi;
		var tbStatus = 1;
		if (treeNode.status == "deleted") {
			tbStatus = 0;
		}
		var tbInputProp = treeNode.isInputProp;
		var tbAttrType = 1;
			...
			buttons: [ 
				{
					...
					click: function() {
						...
						$.ajax({
							url: ctx + "/system/taobao/mapping/attr/save",
							type:"post",
							async:false, 
							data:{
								...
								"tbKeyProp": tbKeyProp ? 1 : 0,
								"tbSalesProp": tbSalesProp ? 1 : 0,
								"tbColorProp": tbColorProp ? 1 : 0,
								"tbEnumProp": tbEnumProp ? 1 : 0,
								"tbItemProp": tbItemProp ? 1 : 0,
								"tbReqProp": tbReqProp ? 1 : 0,
								"tbMulti": tbMulti ? 1 : 0,
								"tbStatus": tbStatus,
								"tbInputProp": tbInputProp ? 1 : 0,
								"tbAttrType": tbAttrType,
			...
			buttons: [ 
				{
					...
					click: function() {
						...
						$.ajax({
							url: ctx + "/system/taobao/mapping/attrval/save",
							type:"post",
							async:false, 
							data:{
								"channelCid": tbCategoryId,
					...
					buttons: [ 
						{
							...
							click: function() {
								...
								$.ajax({
									url: ctx + "/system/taobao/mapping/category/save",
									type:"post",
									async:false, 
									data:{
										"channelCid": tbCategoryId,
			...
			buttons: [ 
				{
					...
					click: function() {
						...
						$.ajax({
							url: ctx + "/system/taobao/mapping/category/update",
							type:"post",
							async:false, 
							data:{
	...
	function categoryDelete(categoryMappingId) {
		if (confirm("确认删除?")) {
			var applicationId = $("#selectApp").val();
			$.ajax({
				url:ctx + "/system/taobao/mapping/category/delete",
				type:'post',
				async:false, 
				data:{
					'id':categoryMappingId
			...
			buttons: [ 
				{
					...
					click: function() {
						...
						$.ajax({
							url: ctx + "/system/taobao/mapping/attr/update",
							type:"post",
							async:false, 
							data:{
								"id":attrMappingId,
	...
	function attrDelete(attrMappingId) {
		if (confirm("确认删除?")) {
			var applicationId = $("#selectApp").val();
			$.ajax({
				url:ctx + "/system/taobao/mapping/attr/delete",
				type:'post',
				async:false, 
				data:{
					'id':attrMappingId
			...
			buttons: [ 
				{
					...
					click: function() {
						...
						$.ajax({
							url: ctx + "/system/taobao/mapping/attrval/update",
							type:"post",
							async:false, 
							data:{
								"id":attrValMappingId,
	...
	function attrValDelete(attrValMappingId) {
		if (confirm("确认删除?")) {
			var applicationId = $("#selectApp").val();
			$.ajax({
				url:ctx + "/system/taobao/mapping/attrval/delete",
				type:'post',
				async:false, 
				data:{
					'id':attrValMappingId
	...
	function attributeMapping() {
		...
		var attrSetting = {
			data: {
	  		...
	  		async: {
				enable: true,
				url: ctx + "/system/taobao/mapping/attrval/get",
				autoParam:["cid","pid"],
		...
		if (nodes && nodes.length>0) {
			if (nodes[0].isParent) {
				...
				var tbCategoryId = nodes[0].cid;
				$.ajax({
					url: ctx + "/system/taobao/mapping/attr/get",
					type:"post",
					async:false, 
					data:{
						"channelCid": tbCategoryId,
	...
	$(document).ready(function(){
		...
		$("#selectApp").change(function() {
			...
			var categorySetting = {
				...
				async: {
					enable: true,
					url: ctx + "/system/taobao/mapping/category/get",
					autoParam:["cid"],
					otherParam:{"applicationId":applicationId},
					dataFilter: filter
				},
		...
		jQuery(mappedCategory_selector).jqGrid({
			url: ctx + "/system/taobao/mapping/category/loadCategoryMapping",
			mtype: "POST", 					 
			datatype: "json",
			height: "auto",
		...
		jQuery(mappedAttr_selector).jqGrid({
			url: ctx + "/system/taobao/mapping/attr/loadAttrMapping",
			mtype: "POST", 					 
			datatype: "json",
			height: "auto",
			colNames:['序号','渠道类目Id','渠道属性id','渠道属性名称','对应的系统属性代号','默认值','属性类型','是否是关键属性','是否是销售属性','是否是颜色属性','是否是枚举属性','是否是商品属性','是否是多选属性','是否是输入属性','是否必填','状态','操作'],
			colModel:[
			    {name:'id',index:'id',width:30,fixed:true,sortable:false,resize:false,hidden: true,editable: false,align: "center"},
				...
				{name:'attributeType',index:'attributeType',search: false, sortable:false,align:'center',width:'80'},
				{name:'keyProperty',index:'keyProperty',search: false, sortable:false,align:'center',width:'80'},
				{name:'saleProperty',index:'saleProperty',search: false, sortable:false,align:'center',width:'80'},
				{name:'colorProperty',index:'colorProperty',search: false, sortable:false,align:'center',width:'80'},
				{name:'enumprop',index:'enumprop', sortable:false,search: false,align:'center',width:'80'},
				{name:'itemprop',index:'itemprop', sortable:false,search: false,align:'center',width:'80'},
				{name:'multi',index:'multi', sortable:false,search: false,align:'center',width:'80'},
				{name:'inputprop',index:'inputprop', sortable:false,search: false,align:'center',width:'80'},
				{name:'required',index:'required', sortable:false,search: false,align:'center',width:'80'},
				{name:'status',index:'status', sortable:false,search: false,align:'center',width:'80'},
		...
		jQuery(mappedAttrVal_selector).jqGrid({
			url: ctx + "/system/taobao/mapping/attrval/loadAttrValMapping",
			mtype: "POST", 					 
			datatype: "json",
			//width: "100",
			height: "auto",
			colNames:['id','渠道类目Id','渠道属性id','渠道属性值id','渠道属性值','系统属性代号','系统属性值代号','系统属性值','操作'],
			colModel:[
				...
				{name:'attributeValueCode',index:'attributeValueCode', sortable:false,search: false,align:'center',width:'110'},
