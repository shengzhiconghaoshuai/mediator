...
public class ProductChannelOffShelfTask extends ProductTask {
	
	private static final String PRODUCT_CHANNEL_OFF_SHELF_TYPE = "productChannelOffShelf";
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ProductChannelOffShelfTask.class);

	static {
		TaskManager.registTask(PRODUCT_TYPE, PRODUCT_CHANNEL_OFF_SHELF_TYPE, ProductChannelOffShelfTask.class);
	}
	...
	public void doTask() throws ApplicationException {
		...
		if (null != product) {
			...
			productFacadeClient.downShelf(product, context, this.id);
		}
	}
